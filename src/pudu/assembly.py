...
class sbol2_DNA_Assembly(DNA_assembly):
    '''
    ...
    def __init__(self, assemblies:List[Dict],
        *args, **kwargs):
        ...
        for assembly in self.assemblies:
            ...
            if fnmatch(receiver, self.pattern_odd):
                self.has_odd = True
                combinations_list = self.odd_combinations
            elif fnmatch(receiver, self.pattern_even):
                self.has_even = True
                combinations_list = self.even_combinations
            else:
                continue  # Skip assemblies that don't match
            
            for role, parts in assembly.items():
                if role == 'receiver':
                    continue  # Skip receiver key
                parts_per_role = [parts] if isinstance(parts, str) else parts
                self.parts_set.update(parts_per_role)
                list_of_list_of_parts_per_role.append(parts_per_role)
        
            combinations_list.extend(product(*list_of_list_of_parts_per_role))
